...
function App() {
  ...
  return <main class="shell">
    ...
    {workflowId && <section class="editor" data-testid="editor" data-workflow-id={workflowId} data-draft-version={draft?.draft_version ?? -1}><div class="editor-head"><div><p class="eyebrow">Mutable Draft</p><h2>{draft?.name ?? workflowId}</h2><code>{workflowId}</code></div><div class={`lease ${editing?.role ?? "waiting"}`} data-testid="lease-role"><strong>{editing?.role === "holder" ? "Lease holder" : editing?.role === "read_only" ? "Read only" : "Lease available"}</strong><span>generation {editing?.lease_generation ?? "—"}</span>{editing?.holder && <small>{editing.holder.label} · expires {new Date(editing.holder.expires_at).toLocaleTimeString()}</small>}</div></div>
      ...
      <LargeEditor workflowId={workflowId} />
      ...
      <section class="run-panel" data-testid="run-panel" aria-labelledby="run-title">
        ...
        {run ? <div class="run-evidence">
          ...
          {run.generation && <section class="generation-card" data-testid="generation-progress" aria-labelledby="generation-progress-title">
            ...
            {run.generation.branch && <div class="transform-summary" data-testid="branch-progress"><div><span class="transform-mark" aria-hidden="true">B</span><div><strong>Branch counts</strong><small>{run.generation.branch.true_count.toLocaleString()} true · {run.generation.branch.false_count.toLocaleString()} false · exactly one route per item</small></div></div><code>{shortIdentity(run.generation.branch.stream_digest)}</code></div>}
            {run.generation.merge && <div class="transform-summary" data-testid="merge-progress"><div><span class="transform-mark" aria-hidden="true">M</span><div><strong>Merged {run.generation.merge.output_count.toLocaleString()} items</strong><small>{run.generation.merge.true_count.toLocaleString()} true + {run.generation.merge.false_count.toLocaleString()} false · {formatBytes(run.generation.merge.physical_spool_bytes)} bounded spool</small></div></div><code>{shortIdentity(run.generation.merge.stream_digest)}</code></div>}
            {run.generation.summary && <div class="transform-summary" data-testid="summarize-progress"><div><span class="transform-mark" aria-hidden="true">S</span><div><strong>Summary: {run.generation.summary.total_count.toLocaleString()} items</strong><small>{run.generation.summary.true_count.toLocaleString()} true · {run.generation.summary.false_count.toLocaleString()} false · {formatBytes(run.generation.summary.logical_bytes)} logical bytes · {run.generation.summary.retained_segments.length} retained Merge segment{run.generation.summary.retained_segments.length === 1 ? "" : "s"}</small></div></div><code>{shortIdentity(run.generation.summary.output_digest)}</code></div>}
...
function formatMicros(value: number | null | undefined): string {
  ...
  if (value < 1_000) return `${value.toLocaleString()} µs`;
