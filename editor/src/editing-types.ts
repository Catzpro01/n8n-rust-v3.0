...
export type RunView = {
  ...
  live?: { state: string; speculative: boolean; boot_epoch: string; sequence: number; generated_count?: number };
  recovery?: { state: "recovering" | "replaying" | "running" | "completed" | "failed" | "cancelled"; attempt: number; recovered_from_checkpoint: number; resume_cursor: number; replay_window: { first_ordinal: number; last_ordinal: number; maximum_items: number; replayed_items: number }; pinned_revision_id: string; pinned_plan_digest: string; artifact_reference_count: number; detected_at: number; restart_elapsed_millis: number; objective_millis: number; within_objective: boolean; sqlite: { journal_mode: string; synchronous: string }; evidence: { checkpoint_commit_count: number; trace_event_count: number; artifact_bytes_written: number } };
